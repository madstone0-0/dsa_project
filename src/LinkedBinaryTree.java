...
public class LinkedBinaryTree<T> extends AbBinaryTree<T> {
  ...
  protected BinaryTreeNode<T> createNode(T t, BinaryTreeNode<T> parent,
  ...
  protected BinaryTreeNode<T> validate(TreeNode<T> n)
    ...
    if (!(n instanceof BinaryTreeNode))
      throw new IllegalArgumentException("Not a valid node type");
  ...
  @Override
  public BinaryTreeNode<T> right(TreeNode<T> n)
      throws IllegalArgumentException {
    var node = validate(n);
  ...
  public void attach(BinaryTreeNode<T> n, LinkedBinaryTree<T> left,
  ...
  public static void main(String[] args) {
    ...
    var left = tree.addLeft(root, 12);
    var right = tree.addRight(root, 13);
    tree.addLeft(left, 14);
    tree.addLeft(right, 16);
    tree.addRight(right, 17);
    var sib = tree.sibling(left);
    assert right == sib;
    tree.remove(left);
    tree.depth(root);
    tree.height(root);
    tree.depth(right.left);
  }
}
